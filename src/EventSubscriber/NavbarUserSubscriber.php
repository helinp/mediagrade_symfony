...
class NavbarUserSubscriber implements EventSubscriberInterface
{
    ...
    public function onShowUser(ShowUserEvent $event)
    {
        ...
        $title = TitleFromRole::getTitleFromRoles($roles);

        $user
            ->setId($myUser->getId())
            ->setName($myUser->getDisplayName())
            ->setUsername($myUser->getUsername())
            ->setIsOnline(true)
            ->setTitle($title)
            ->setAvatar(($myUser->getUserAvatar() ? '/uploads/profile/' . $myUser->getUserAvatar()->getImageName() : '/bundles/adminlte/images/default_avatar.png'))
