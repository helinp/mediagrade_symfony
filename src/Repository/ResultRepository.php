...
public function getStudentResultByTerms($student, $shoolyear)
{
	...
	->select('SUM(r.userVote) / SUM(r.maxVote) * 100 AS percentage, t.name, t.description, t.id as term_id')
...
public function getStudentResultByCourseAndTerm($course, $student, $shoolyear, $term_id)
{
	...
	->andWhere('p.assessmentType <> ' . AssessmentType::TYPE_DIAGNOSTIC)
	->groupBy('t.id')
	->getQuery()
	->getOneOrNullResult()
	;
...
public function getSkillsGroupsResultByTermAndStudent($term, $student, $schoolyear)
{
	return $this->createQueryBuilder('r')
